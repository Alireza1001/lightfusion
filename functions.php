...
function multithumbmail_html( $post ) { 
	...
	foreach( get_posts( $args ) as $image) {
		...
		echo 
			'<li data-id="'.$image->ID.'" data-url="'.$imgmainsrc.'">
				<img src="'.$newimgsrcset1.'">
			</li>';
	}
	echo '
			</ul>
	...
	<img src="" id="displayattachment_preview">
	...
	<?php
		...
		<script>
			...
			document.querySelectorAll("#secondthumbnail_list li").forEach(item=>{
				item.addEventListener("click", ()=>{
					console.log(item.getAttribute("data-url"));
					document.getElementById("multithumbmail_n").value = item.getAttribute("data-id");
					document.getElementById("displayattachment_preview").src = item.getAttribute("data-url");
					item.classList.add("active");
				});
			});

			const thedom = document.querySelector("#secondthumbnail_cover");
			document.querySelector("#displayattachmentslist").addEventListener("click", ()=>{
				if(thedom.classList.contains("active")) {
			...
			document.querySelector("#close_secondthumbnail").addEventListener("click", ()=>{
				thedom.classList.remove("active");
				document.body.classList.remove("active");
			});
