            ...
            <?php
                if ( have_posts() ) :
                    while ( have_posts() ) :
                        ...
                        echo '<div class="lf_item '.str_replace(" ", "", get_the_category()[0]->name).' '.str_replace(" ", "", get_the_category()[1]->name).'"><a href="'.getTheLink($post).'">'.$imgmaintag.'</a><div class="lf_item_profile"><div><img src="'.$tempalte_dir."/assets/images/authors/".$user_id.".jpg".'"/><p>By:<span>'.get_the_author() .'</span></p></div><p>'. get_the_date() .'</p></div><div class="lf_item_content"><h2><a href="'. getTheLink($post) .'">'. get_the_title() .'</a></h2><div class="lf_item_cat">'.$theget_the_categorypost.'</div><p>'. get_the_excerpt() .'</p></div><div class="lf_item_bottom"><a href="'. getTheLink($post) .'">read this article</a><p><img src="/wp-content/themes/lightfusion/assets/icons/comment-dark.svg" />'. get_comments_number() .'</p></div></div>';
                    endwhile;
                    wp_reset_postdata();
                endif;
            ?>
